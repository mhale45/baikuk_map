    ...
    if (staffInfoBox) {
      ...
      if (mainLogo) {
        ...
        if (aff === '스타운정점') {
          ...
          mainLogo.style.marginTop = '1.9rem';
        } else {
...
async function loadCustomerDataByName(name, list_name = null) {
  ...
  if (hasListName) {
    ...
    // 리스트 이름이 없거나 빈 문자열인 경우 첫 번째 레코드를 무사히 획득하도록 limit(1) 처리
...
function updateSaveButtonState() {
  ...
  if (currentCustomerId) {
    ...
    saveBtn.className = "h-[2rem] w-[4.5rem] bg-white border border-blue-500 text-blue-500 hover:bg-blue-50 text-sm rounded font-bold transition-all select-none";
  } else {
    saveBtn.textContent = "저장";
    saveBtn.className = "h-[2rem] w-[4.5rem] bg-blue-500 hover:bg-blue-600 text-white text-sm rounded font-bold transition-all select-none";
  }
}
