...
function renderMonthlyTable({ titleAffiliation, salesMap, payrollByStaff, costMap, staffList }) {
  ...
  for (const ym of yms) {
    ...
    tr.innerHTML = `
      ...
      <td class="border px-2 py-2 text-right font-semibold text-red-600">${fmt(vat)}</td>
      <td class="border px-2 py-2 text-right">${fmt(realVat)}</td>
      ...
      <td class="border px-2 py-2 text-right font-semibold text-green-500">${fmt(dispFinalProfit)}</td>
    `;
