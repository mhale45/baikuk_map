    ...
    listings.forEach(item => {
        ...
        html += `
            <div class="listing-item" data-id="${item.listing_id}" style="padding:4px 0; font-size:14px; cursor:pointer; ${bgColor}">
                ...
                <strong><span style="color:blue; min-width:65px; text-align:right;">보 </span>${formatNumber(item.deposit_price)}</strong> /
                <strong><span style="color:green; min-width:55px; text-align:right;">월 </span>${formatNumber(item.monthly_rent)}</strong> /
                ${
                    (!item.premium_price || Number(item.premium_price) === 0)
                        ? `<strong><span style="color:red; min-width:65px; text-align:right;">무권리</span></strong> /`
                        : `<span style="min-width:65px; text-align:right;"><strong><span style="color:red;">권 </span></strong> <strong>${formatNumber(item.premium_price)}</strong></span> /`
                }
                ${
                    item.rent_per_py
